import { initDatabase, saveStateToDB, downloadDatabase, loadDatabaseFile } from "./database.js";
import { createNode, makeDraggable, selectNode } from "./nodes.js";
import { createRow, removeRow } from "./items.js";
import { updateConnections } from "./connections.js";

const $ = id => document.getElementById(id);
let nodes = [], connections = [];
let selectedNode = null, currentEditItem = null, connectionSelectionMode = false, selectedConnectionTarget = null;
let dbInstance = null;
...
dbFileInput.addEventListener("change", e => {
  if (e.target.files.length)
    loadDatabaseFile(e.target.files[0], newDb => { dbInstance = newDb; loadStateFromDBToUI(); });
});
addNodeButton.addEventListener("click", () => createNode(100, 100, editor, nodes, makeDraggable));
...
editor.addEventListener("click", e => {
  if (e.target === editor) {
    selectedNode?.classList.remove("border-2", "border-blue-500");
    selectedNode = null;
...
const buildConditionRow = (cond = {}) => {
  ...
  const variable = Object.assign(document.createElement("input"), { type: "text", placeholder: "Variable", className: "border p-1", value: cond.variable || "" });
  ...
  const value = Object.assign(document.createElement("input"), { type: "text", placeholder: "Value", className: "border p-1", value: cond.value || "" });
...
const buildFlagRow = (flag = {}) => {
  const row = document.createElement("div");
  row.className = "flex space-x-2 items-center";
  const flagName = Object.assign(document.createElement("input"), { type: "text", placeholder: "Flag Name", className: "border p-1", value: flag.flagName || "" });
  const label = document.createElement("label");
  label.className = "flex items-center space-x-1";
  const checkbox = Object.assign(document.createElement("input"), { type: "checkbox", checked: flag.value || false });
...
addItemButton.addEventListener("click", () => {
  ...
  editItemModal.classList.remove("hidden");
  currentEditItem = { nodeId: selectedNode.dataset.id };
});
...
document.addEventListener("editItem", e => {
  ...
  currentEditItem = { nodeId: row.parentElement.parentElement.dataset.id, editingRow: row };
  editItemModal.classList.remove("hidden");
...
confirmItemBtn.addEventListener("click", () => {
  ...
  const conditions = [...conditionsList.children].map(div => {
    ...
    return { variable: variable.value, operator: operator.value, value: value.value };
  });
  const flags = [...flagsList.children].map(div => {
    const [flagName, checkbox] = div.querySelectorAll("input");
    return { flagName: flagName.value, value: checkbox.checked };
  });
  const title = itemTitleInput.value;
  const details = { title, conditions, flags, connectionTarget: selectedConnectionTarget };
...
function loadStateFromDBToUI() {
  ...
  dbInstance.exec("SELECT * FROM nodes")[0]?.values.forEach(([id, x, y, title]) =>
    createNode(x, y, editor, nodes, makeDraggable, { id, title })
  );
  ...
  dbInstance.exec("SELECT * FROM connections")[0]?.values.forEach(([connId, fromNodeId, fromItemId, toNodeId]) => {
    ...
    connections.push({ from: { nodeId: fromNodeId, itemId: fromRow.row.dataset.itemId }, to: { nodeId: toNodeId }, line });
  });
